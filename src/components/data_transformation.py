...
class DataTransformation:
    ...
    def initiate_data_transformation(self, train_path, test_path):
        try:
            ...
            preprocessing_obj: ColumnTransformer = self.get_data_transformer_object()
            ...
            target_feature_train_df: pd.Series[float] = train_df[target_column_name]

            input_feature_test_df: pd.DataFrame = test_df.drop(
                columns=[target_column_name], axis=1
            )
            target_feature_test_df: pd.Series[float] = test_df[target_column_name]

            logging.info(
                "Applying the preprocesing object on the training dataset and test dataset"
            )
